 ...
 * ================================================
 */
public class TransactionFragment extends BaseSupportFragment<StoreManagerPresenter> implements StoreManagerContract.View {

    @Inject
    List<Transaction> transactionList;

    @Inject
    TransactionQuickAdapter transactionQuickAdapter;
    ...
    public static TransactionFragment newInstance(String storeId,int type) {
    ...
    private void initRecycler() {
        ...
        transactionQuickAdapter.setEmptyView(LayoutInflater.from(getActivity()).inflate(R.layout.view_empty, null));
        transactionQuickAdapter.setOnItemClickListener((adapter, view, position) -> {

                }
        );

        transactionQuickAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            @Override
        ...
        mRecyclerView.setAdapter(transactionQuickAdapter);
    }
    ...
    private void request(){
        ...
        mPresenter.transaction(BaseApp.loginBean.getToken(),storeId,mNextRequestPage,type);

    }

    private void refresh() {
        transactionQuickAdapter.loadMoreEnd(true);
        mNextRequestPage = 1;
        transactionQuickAdapter.setEnableLoadMore(false);//这里的作用是防止下拉刷新的时候还可以上拉加载
        request();
    ...
    private void loadMore() {
        if(mNextRequestPage<pages){
            ...
            transactionQuickAdapter.loadMoreEnd();
        }
    ...
    private void setData(boolean isRefresh, List data) {
        ...
        if (isRefresh) {
            transactionQuickAdapter.setNewData(data);
        } else {
            if (size > 0) {
                transactionQuickAdapter.addData(data);
            }
        }
        if (size < PAGE_SIZE) {
            //第一页如果不够一页就不显示没有更多数据布局
            transactionQuickAdapter.loadMoreEnd(isRefresh);
            //Toast.makeText(this, "no more data", Toast.LENGTH_SHORT).show();
        } else {
            transactionQuickAdapter.loadMoreComplete();
        }
    }
    ...
    public void transactionSuccess(TransList data) {
        ...
        if(isRefresh){
            ...
            transactionQuickAdapter.setEnableLoadMore(true);
            swipe_refresh.setRefreshing(false);
        ...
        if(data.entityList==null){
            ...
            setData(isRefresh, data.entityList);
        }
    }
