...
public class HomeFragment extends BaseSupportFragment<HomePresenter> implements HomeContract.View {
    ...
    private void setupViewPager(ViewPager viewPager,List<CatBean> data) {
        adapter = new Adapter(getChildFragmentManager());

        adapter.addFragment(new RecommendFragment(), "推荐");
        for(CatBean cat : data){

            String name = cat.categoryName;
            adapter.addFragment(CatProductFragment.newInstance(cat.id,name), name);
