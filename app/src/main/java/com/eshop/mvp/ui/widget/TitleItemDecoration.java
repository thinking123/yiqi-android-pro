...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, List<BrandBean> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).brandZm && !mData.get(position)
                            .brandZm.equals(mData.get(position - 1).brandZm)) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).brandZm, 0, mData.get(position).brandZm.length(), mBounds);
        c.drawText(mData.get(position).brandZm,
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).brandZm &&
                        !mData.get(position).brandZm.equals(mData.get(position - 1).brandZm)) {
